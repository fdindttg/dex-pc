          ...
          <template v-if="step == 1">
            <ul class="nav nav-tabs" id="myTab" role="tablist">
              <li class="nav-item" @click="registerType = 1">
                <a
                  :class="[`nav-link`, { active: registerType == 1 }]"
                  id="email-tab"
                  data-toggle="tab"
                  href="#email"
                  role="tab"
                  aria-controls="email"
                  :aria-selected="registerType == 1"
                  >{{ $t('login.email') }}</a
                >
              </li>
              <!-- <li class="nav-item" v-if="true" @click="registerType = 2">
                <a
                  :class="[`nav-link`, { active: registerType == 2 }]"
                  id="phone-tab"
                  data-toggle="tab"
                  href="#phone"
                  role="tab"
                  aria-controls="phone"
                  :aria-selected="registerType == 2"
                  >{{ $t('login.phone') }}</a
                >
              </li> -->
            </ul>
            ...
            <div class="tab-content" id="myTabContent">
              ...
              <div
                class="tab-pane fade"
                :class="{ show: registerType == 1, active: registerType == 1 }"
                v-if="registerType == 1"
                id="email"
                role="tabpanel"
                  ...
                  <div class="d-flex">
                    <div class="mr-2">
                      <el-select v-model="user.country_id" filterable :placeholder="$t('login.country')">
                        <el-option
                          v-for="(item, index) in cacheOption"
                          :key="index"
                          :value="`${item.id}`"
                    ...
                    <input
                      ...
                      v-model="user.phone"
                      :data-has="$t('login.enterMobile')"
                      pattern="^\d{9,13}$"
                      :data-message="$t('login.invalidPhone')"
                    />
                  </div>
                </div>
              </div>
            ...
            <div class="form-group row">
              <input
                type="text"
                class="form-control"
                v-model="user.parentCode"
                :placeholder="`${$t('login.referrer')}（${$t('login.optional')}）`"
              />
            </div>
            ...
            <div class="custom-control custom-checkbox">
              ...
              <label class="custom-control-label" for="form-checkbox">
                {{ $t('login.iSee') }}
                <a href="#/service/10">{{ $t('login.terms') }}</a>
              </label>
            </div>

            <button type="submit" class="btn btn-primary rounded-pill" @click.prevent.stop="step1">
              {{ $t('login.reg') }}
            </button>
          </template>
          ...
          <template v-if="step == 3">
            <div v-if="registerType === 1" class="verify-code">
              <div class="form-text text-muted">
                ...
                <br />
                {{ $t('login.accountActivated') }},<a href @click.prevent="step = 1">{{ $t('login.backOld') }}</a>
              </div>
              <section class="my-codes">
                ...
                <div class="my-codes__controls">
                  <input
                    ...
                    ref="spaces"
                    class="form-control"
                    @keyup="handleKeyUp(pos - 1, $event)"
                    @input="handleInput(pos - 1, $event)"
                    @paste="autofill"
                  />
                </div>
              </section>
          ...
          <iframe
            ...
            :src="
              `/static/SliderCaptcha/src/index.html?` +
                encodeURIComponent(`nextStep=step2Register&slider_type=register&account=${account}&lang=${lang}`)
            "
            frameborder="0"
          ></iframe>
        </div>
...
export default {
  ...
  computed: {
    ...
    // 当前选中的区号
    country() {
      return this.cacheOption.find((item) => item.id == this.user.country_id) || {}
    }
  },
      ...
      if (utils.validate('.register-body')) {
        if (!this.user.country_id) {
          this.$message(
            {
              type: 'error',
              message: this.$t('login.phoneCorrect') + '！'
            },
            1000
          )
          return
      ...
      switch (
        ...
        case 2: // 发送手机验证码
          Member.sendSmsCode({
            ...
            .then((data) => {
              this.ajar.phone = this.user.phone.replace(/^(\d{3})\d{4}(\d{4})$/, '$1****$2')
              this.step = 3
              // 自动第一个焦点
              this.$nextTick(function() {
                // DOM 现在更新了
    ...
    if (this.$route.query.invite_code) {
      ...
      if (document.body.clientWidth < 750) {
        window.location.href =
          this.Globals.mobileUrl + '/#/pages/reg/index?invite_code=' + this.$route.query.invite_code
      }
    }
  },
...
div.verify-code {
  ...
  .prompt {
    .prompt__no-recieved {
      @include font(Overpass-Regular, 13, 30, #ddd);
    }

    .prompt__inner {
      @include font(Overpass-Regular, 12, 20, #ddd);
      @extend .form-text;
      @extend .text-muted;
    }
  }
