        ...
        <div class="row py-5">
            ...
            <div>
              <ul class="list">
                <li class="d-flex align-items-start my-3">
                  ...
                  <div>
                    <div class="label h5">{{ $t('college.register') }} Bekon</div>
                    <div class="txt">{{ $t('college.visit', { name: 'Bekon' }) }}</div>
                  </div>
                </li>
                <li class="d-flex align-items-start my-3">
          ...
          <div class="col-7 right" v-if="video">
            <video
              ...
              controls
              :src="video.url"
              :poster="siteUrl + `/storage/` + video.poster"
            ></video>
        ...
        <div class="container">
          <div class="row">
            <div class="col-12 d-flex">
              <div
                class="w-25 item active"
                v-for="(item, index) in bannerList"
                :key="index"
                @click="fnView(item, index)"
              >
...
export default {
  data() {
    return {
      siteUrl: this.Globals.Server.Path.BASE,
      // 当前播放的视频
      video: null,
      categoryList: [],
      bannerList: []
  ...
  methods: {
    ...
    initPage() {
      College.college()
        .then((response) => {
          ;({ categoryList: this.categoryList, bannerList: this.bannerList } = response)
