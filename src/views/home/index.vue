      ...
      <div class="marquee">
        <div class="marquee-pargress d-flex">
          <ul class="marquee-list d-flex" v-for="box in 4" :key="box">
            <li class="item border-right" v-for="item in homeList" :key="item.pair + '' + box">
      ...
      <div class="new-ticker-block new-ticker-block-section">
        <div class="container">
            ...
            <div class="ticker-head">
              <ul class="nav nav-tabs ticker-nav" role="tablist">
                ...
                <li class="nav-item" @click="tab = item.coin_name" v-for="item in marketList" :key="item.coin_name">
                  <a class="nav-link" :class="{ active: tab == item.coin_name }" href="javascript:void(0);">{{
                    item.coin_name
                  }}</a>
                </li>
              </ul>
                  ...
                  <table class="table coin-list table-hover">
                    ...
                    <tbody>
                      <tr v-for="item in collect" v-show="isShow(item.pair_name)" :key="item.pair_name">
                        ...
                        <td>
                          <router-link :to="`/exchange/${item.symbol}`">{{ item.pair_name }}</router-link>
                        </td>
                        <td>
                  ...
                  <table class="table coin-list table-hover">
                    ...
                    <tbody>
                      <template v-for="(item, idx) in parentItem.marketInfoList">
                        <tr v-if="idx < 8" v-show="isShow(item.pair_name)" :key="item.pair_name">
                          <td>
                            <i
                              href="javascript:void(0);"
                              @click="option(item)"
                          ...
                          <td>
                            <span :class="item.increase * 1 >= 0 ? 'increace' : 'decreace'">{{
                              item.increaseStr
                            }}</span>
                          </td>
                          <td>{{ item.high }}</td>
          ...
          <div class="text-center">
            <router-link to="/market" tag="button" type="button" class="view-more btn btn-outline-light">
              {{ $t('home.more') }}
            </router-link>
          </div>
        </div>
      ...
      <aside class="activity-box">
        <div class="swiper-container">
          <ul class="swiper-wrapper">
            <li :class="[`swiper-slide`, { zoom: item.zoom }]" v-for="(item, index) in featuresList" :key="index">
              <!-- 鼠标移上去效果 -->
          ...
          <div class="left text-center d-flex align-items-center">
            <div class="d-flex flex-column align-items-center">
              ...
              <div class="qr-code-box">
                ...
                <div class="label">
                  <img class="scan-icon" src="../../assets/img/homescan.png" alt="" />
                  {{ $t('home.skan') }}
                </div>
              </div>
            </div>
        ...
        <div class="container">
          ...
          <div class="row content">
            <router-link
              ...
              tag="div"
              :to="{ name: 'college-detail', params: { cid: item.category_id, aid: item.id } }"
              class="item rounded"
              :key="idx"
            >
              <figure>
...
export default {
  ...
  mounted() {
    console.log(111, this.Globals.Server.Path.BASE)
    this.indexList()
    if (this.isLogin) {
      this.getCollect()
    }
