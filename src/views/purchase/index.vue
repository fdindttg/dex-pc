        ...
        <div class="col-6 d-flex flex-column justify-content-end align-items-start">
          <div class="input-group mb-3">
            <div class="input-group-prepend">
              <el-dropdown>
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item v-for="item in coinList" @click.native="activeCoin = item" :key="item.coin_name">{{
                    item.coin_name
                  }}</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
            ...
            <div class="input-group-append">
              <button class="btn btn-outline-primary" type="button" @click="num = activeCoin.usable_balance">
                {{ $t('purchase.all') }}
              </button>
            </div>
          </div>
          <p v-if="activeCoin.usable_balance">{{ $t('wallet.availableBalance') }}：{{ activeCoin.usable_balance }}</p>
          <p>
          ...
          <p>
            {{ num || 0 }} {{ activeCoin.coin_name }} ≈ {{ total | omitTo(5) }}
            {{ detail.coin_name }}
          </p>
          <button type="submit" class="btn btn-primary rounded-pill" @click="ifSubscribeNow">
      ...
      <div class="row">
        <div class="col-12">
          ...
          <el-steps :active="activestep + 1" align-center>
            <el-step :description="$t('nav.c2')"></el-step>
            <el-step
              :description="
                $t('nav.c1', {
                  amount: activityList[0].amount,
                  name: detail.coin_name,
                  rate: activityList[0].rate * 100
                })
              "
            ></el-step>
            <el-step
              :description="
                $t('nav.c1', {
                  amount: activityList[1].amount,
                  name: detail.coin_name,
                  rate: activityList[1].rate * 100
                })
              "
            ></el-step>
            <el-step
              :description="
                $t('nav.c1', {
                  amount: activityList[2].amount,
                  name: detail.coin_name,
                  rate: activityList[2].rate * 100
                })
              "
            ></el-step>
            <el-step
              :description="
                $t('nav.c1', {
                  amount: activityList[3].amount,
                  name: detail.coin_name,
                  rate: activityList[3].rate * 100
                })
              "
            ></el-step>
            <el-step
              :description="
                $t('nav.c1', {
                  amount: activityList[4].amount,
                  name: detail.coin_name,
                  rate: activityList[4].rate * 100
                })
              "
            ></el-step>
          </el-steps>
          ...
          <div class="tab-content">
            <div class="tab-pane fade show active" id="tab1" role="tabpanel">
              <table class="table">
                ...
                <tbody>
                  <template v-for="item in subscribeRecords">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-panel">
          ...
          <div class="modal-body">
            <form>
              <div class="form-group">
                <label for="comments" class="col-form-label">{{ $t('contract.k9') }}:</label>
                <input type="text" class="form-control" id="comments" v-model="code" />
              </div>
            </form>
          </div>
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary rounded-pill" @click="sbumit">
              {{ $t('common.confirmBtn') }}
            </button>
          </div>
        </div>
...
export default {
  ...
  computed: {
    total() {
      return utils.multiple(this.num || 0, this.activeCoin.proportion_amount * 1, 5)
    },
  ...
  methods: {
    ...
    ifSubscribeNow() {
      if (this.chkValidate()) {
        this.$confirm(
          `${this.$t('purchase.useOrNot')}${this.activeCoin.coin_name}${this.$t('purchase.subscribe')}?`,
