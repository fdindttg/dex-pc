<template>
  <div class="markets-pair-list exchange-store bg-plain">
    <div class="nav nav-pills">
      <div class="nav-item">
        <a class="nav-link" :class="{ active: tab == 1 }" @click="tab = 1">{{ $t('contract.d5') }}</a>
      </div>
      <div class="nav-item">
        <a class="nav-link" :class="{ active: tab == 2 }" @click="tab = 2">{{ $t('contract.d6') }}</a>
      </div>
    ...
    <div class="content-box px-3 ">
      ...
      <div class="" v-if="tab == 1">
        <div class="px-2 flex-fill mb-4">
          ...
          <div class="text-center">{{ $t('contract.e7') }} {{ maxNum }} {{ $t('contract.e2') }}</div>
        </div>
      ...
      <div class="" v-if="tab == 2">
        <div class="px-2 flex-fill mb-4">
          ...
          <div class="text-center">{{ $t('contract.e9') }} {{ canMore }} {{ $t('contract.e2') }}</div>
        </div>
        <div class="px-2 flex-fill">
          <button @click="ifOpenPosition(2)" type="button" class="sub w-24/24 btn-danger btn btn-sm">
            {{ $t('contract.d1') }}
          </button>
          <div class="text-center">{{ $t('contract.e9') }} {{ canEmpty }} {{ $t('contract.e2') }}</div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    // 提示
    ifOpenPosition(side) {
      let typeStr = this.form.type == 1 ? `${this.form.entrust_price}${this.$t('contract.f0')}` : this.$t('contract.d9')
      let decStr =
        this.tab == 1
          ? `${this.$t('contract.d5')} ${this.form.amount} ${this.$t('contract.e2')} ${
              side == 1 ? this.$t('contract.f1') : this.$t('contract.f2')
            }`
          : `${this.$t('contract.d6')} ${this.form.amount} ${this.$t('contract.e2')}`
      this.$confirm(`${this.$t('contract.f3')}  ${this.$t('contract.f4')} ${this.form.lever_rate} ${decStr}?`)
        .then((res) => {
          if (this.tab == 1) {
