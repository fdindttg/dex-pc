<template>
  <div>
    <div class="d-flex justify-content-between py-2 px-3">
      ...
      <div class="d-flex align-items-center" @click="$emit('isall', !holdPositionAll)">
        {{ $t('contract.i6') }}
        <input type="checkbox" class="ml-1" :checked="holdPositionAll" style="pointer-events: none" />
      </div>
    </div>
    <table class="table">
      <thead>
      ...
      <tbody>
        <tr v-for="item in showList" :key="item.id">
          ...
          <td>
            <button class="btn btn-sm btn-danger mb-1" @click="liquidation(item)">
              {{ $t('contract.d6') }}
            </button>
            <!-- <button
            ...
            <button class="btn btn-sm btn-primary mb-1" @click="toStoploss(item)">
              {{ $t('contract.i9') }}
            </button>
          </td>
        </tr>
          ...
          <div class="modal-body">
            <form>
              <div class="form-group">
                <label for="recipient-name" class="col-form-label">{{ $t('contract.d8') }}:</label>
                <div class="input-group">
                  ...
                  <div class="input-group-append">
                    <button
                      type="button"
                      @click="activeItem.closeType == 1 ? (activeItem.closeType = 2) : (activeItem.closeType = 1)"
                      class="btn btn-outline-primary"
                    >
                      {{ activeItem.closeType == 1 ? $t('contract.e0') : $t('contract.d9') }}
                    </button>
                  </div>
                </div>
              </div>
              <div class="form-group">
                ...
                <input type="number" v-model="form.amount" class="form-control" id="message-text" />
              </div>
              <div>
                {{ $t('contract.e9') }}
    ...
    >
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">{{ $t('contract.h2') }}{{ activeItem.pair_name }}{{ $t('contract.a9') }}？</div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">
              {{ $t('contract.c4') }}
            </button>
            <button type="button" class="btn btn-primary" @click="closeAllPosition">
              {{ $t('contract.c5') }}
            </button>
          </div>
        </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary" @click="onekeyAllFlat">
              {{ $t('contract.c5') }}
            </button>
          </div>
        </div>
    ...
    <!-- 一键反向 -->
    <div class="modal fade" id="reverse" tabindex="-1" role="dialog" aria-labelledby="reverseLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
          ...
          <div class="modal-body">
            <div class="info border-bottom pb-2">
              <div>
                ...
                <span class="text-success">{{ activeItem.side == 1 ? $t('contract.i4') : $t('contract.i5') }}</span>
              </div>
              <div class="d-flex">
            ...
            <form>
              <div class="form-group">
                ...
                <div class="input-group">
                  <input type="text" class="form-control" v-model="strategyForm.tp_trigger_price" />
                </div>
                <small class="form-text text-muted" v-if="strategyForm.tp_trigger_price">
                  {{ $t('contract.j5') }}{{ strategyForm.tp_trigger_price }}{{ $t('contract.j6') }}
                  {{
                    activeItem.side == 1
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  <input type="text" class="form-control" v-model="strategyForm.sl_trigger_price" />
                </div>
                <small class="form-text text-muted" v-if="strategyForm.sl_trigger_price">
                  {{ $t('contract.j5') }}{{ strategyForm.sl_trigger_price }}{{ $t('contract.j7') }}
                  {{
                    activeItem.side == 1
...
export default {
  ...
  methods: {
    ...
    // 设置数量
    sliderChange($ev) {
      this.form.amount = Math.round((this.activeItem.avail_position * $ev) / 100)
    },
    // 多仓盈亏
    many(item, newPrice) {
    ...
    closeAllPosition() {
      ...
      Contract.closeAllPosition(data).then(() => {
        this.$message.success(`${this.$t('contract.h3')}${this.activeItem.pair_name}${this.$t('contract.h4')}`)
        this.$emit('position')
