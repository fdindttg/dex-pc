...
export default {
    ...
    methods: {
        handle() {
            if(this.consults.category_id === undefined || this.consults.category_id === ''){
                this.$message({
                        type: "error",
                        message: this.$t('contact.pleseSelect')+this.$t('contact.consultingService'),
                    });
                    return;
            }
            if (utils.validate('.cont-body')) {
                ContactUs.contactUs(this.consults).then(response => {
                    this.$message({
                        ...
                        message: this.$t('contact.Submitted'),
                    });
                    // 清空表单
                    this.consults = {
                        realname: '',
