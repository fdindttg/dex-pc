  ...
  <div class="edit-google">
    ...
    <div>
      <div class="form-group" v-if="!detail.google_token">
        ...
        <div class="tip fn-12" style="margin-top: 10px;">
          <!-- <div>{{ $t('setting.howToOpen', { name: 'Google' }) }}?</div> -->
          <!-- <a href="#" class>{{$t('setting.read')}}。</a> -->
          <div>How to set up Google Authenticator:</div>
          <div>A.For IOS users</div>
          <div>1.Go to Apple store and search for Google Authenticator app.</div>
          <div>
            2.Install and open the Google Authenticator, then scan the QR code above or manually enter the secret key to
            add a verification token.
          </div>
          <div>B.For Android users</div>
          <div>1.Go to Google Playstore and search for Google Authenticator app.</div>
          <div>
            2.Install and open the Google Authenticator app, then scan the QR code above or manually enter the secret
            key to add a verification token.
          </div>
        </div>
      </div>
      <div class="form-group" v-if="!detail.google_token">
    ...
    <div class="text-center">
      ...
      <button type="button" class="btn w-25 btn-primary" v-if="!detail.google_token" @click="bindGoogleToken">
        {{ $t('setting.bind') }}
      </button>
      <button type="button" class="btn w-25 btn-primary" v-else @click="unBindGoogle">
        {{ $t('setting.unbind') }}
      </button>
    </div>
  </div>
