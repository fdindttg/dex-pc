  ...
  <div class="setting-page">
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-md-6 d-flex">
          <div class="panel-box bg-panel shadow-sm mb-4 w-100 d-flex flex-column">
            ...
            <div class="tab-content flex-fill d-flex flex-column justify-content-between align-items-start">
              ...
              <button class="btn btn-primary btn-sm" @click="loginShow = true">
                {{ $t('setting.resLoginPassword') }}
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="col-md-6 d-flex">
          <div class="panel-box bg-panel shadow-sm mb-4 w-100 d-flex flex-column">
            ...
            <div class="tab-content flex-fill d-flex flex-column justify-content-between align-items-start">
              ...
              <button class="btn btn-primary btn-sm" @click="transShow = true" v-if="detail.is_set_payword">
                {{ $t('setting.modifyPassword') }}
              </button>
              <button class="btn btn-primary btn-sm" @click="transShow = true" v-else>
                {{ $t('setting.bind') }}
              </button>
            </div>
          </div>
        ...
        <div class="col-md-4 d-flex">
          <div class="panel-box bg-panel shadow-sm mb-4 w-100 d-flex flex-column">
            ...
            <div class="tab-content flex-fill d-flex flex-column justify-content-between align-items-start">
              ...
              <div class="status text-tips my-2" v-if="detail.google_status == 0 && detail.google_token">
                {{ $t('setting.notOpen') }}
              </div>
              <div class="group d-flex align-items-center">
                ...
                <div class="custom-control custom-switch" v-if="detail.google_token">
                  <input
                    type="checkbox"
                    readonly
                    @change="verificationChange(3, detail.google_status)"
                    v-model="detail.google_status"
                    class="custom-control-input"
                    id="GoogleSwitch"
                  />
                  <label class="custom-control-label" v-if="detail.google_status" for="GoogleSwitch">{{
                    $t('setting.close')
                  }}</label>
        ...
        <!-- <div class="col-md-4 d-flex">
                    <div class="panel-box bg-panel shadow-sm mb-4 w-100 d-flex flex-column">
                        <div class="heading">{{$t('common.accountInformation')}}</div>
                        <div class="tab-content flex-fill d-flex flex-column justify-content-between align-items-start">
                            <div>{{$t('common.nickName')}}: </div><div>{{ detail.phone }}</div>
                            <div>{{$t('common.avatar')}}:</div>
                            ...
                            <div class="group d-flex align-items-center">
                                <button class="btn btn-primary btn-sm mr-2" @click="userInfoShow=true">
                                    <template>{{$t('common.updateUserInfo')}}</template>
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    <v-box v-model="loginShow" :title="$t('setting.loginPassword')">
      <login-pwd @close="logout" @change="getUserInfo" :detail="detail" id="loginPwd" />
    </v-box>

    <!-- 校验开关 -->
    <v-box
      v-model="verification"
      :title="`${changeVerificationObj.status ? $t('setting.open') : $t('setting.close')}${changeVerificationObj.name}`"
    >
      <change-verification
        @close="verification = false"
        :changeObj="changeVerificationObj"
        @change="getUserInfo"
        :detail="detail"
      />
    </v-box>
