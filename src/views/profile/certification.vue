        ...
        <div class="col-9 mx-auto mt-4 panel-box shadow-sm bg-panel" id="auth">
            ...
            <div class="form-row">
              <div class="form-group col-md-6">
                ...
                <div class="input-group mb-3">
                  ...
                  <input type="text" required class="form-control" :placeholder="$t('profile.enterPhoneNumber')" />
                </div>
              </div>
            </div>
            <div class="text-center">
              <button type="button" class="btn w-25 btn-primary rounded-pill" @click="primaryAuth">
                {{ $t('profile.nextStep') }}
              </button>
            </div>
          </div>
          ...
          <div class="tab-ceontent p-5 text-center" v-if="setp == 2">
            ...
            <div v-if="detail.status == 1">
              ...
              <div class="text-center">
                <button type="button" class="btn w-25 btn-primary rounded-pill" @click="$router.go(-1)">
                  {{ $t('profile.gotIt') }}
                </button>
              </div>
            </div>
            <div v-else-if="detail.status == 2">
              <p class="mt-3 mb-4">{{ $t('profile.congratulations') }}</p>
            ...
            <div v-else-if="detail.status == 3">
              <p class="mt-3 mb-4">{{ detail.status_text }}！</p>
              <i class="el-icon-s-release my-4 decreace" style="font-size:60px"></i>
              <div class="text-center">
                <button type="button" class="btn w-25 btn-outline-danger" @click="setp = 1">
...
export default {
  ...
  computed: {
    country() {
      return this.countryList.find((item) => item.id == this.form.country_id) || {}
    },
    cardType() {
      return [
        {
  ...
  methods: {
    frontImgInput(val) {
      this.topForm.front_img = val
    },
    backImgInput(val) {
      this.topForm.back_img = val
    },
    headImgInput(val) {
      this.topForm.hand_img = val
    },
    // 获取实名信息
    getAuthInfo() {
      Profile.getAuthInfo()
