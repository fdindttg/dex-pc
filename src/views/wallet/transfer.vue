            ...
            <ul class="nav nav-pills" id="pills-tab" role="tablist">
              <li class="nav-item" v-if="coinName">
                <a
                  ...
                  data-toggle="pill"
                  href
                  @click.prevent="isCondition = true"
                  role="tab"
                  aria-selected="false"
                  >{{ coinName }} {{ $('transfer.transferRecord') }}</a
                >
              </li>
              <li class="nav-item">
                <a
                  ...
                  data-toggle="pill"
                  href
                  @click.prevent="isCondition = false"
                  role="tab"
                  aria-selected="false"
                  >{{ $t('wallet.allTransfer') }}</a
                >
              </li>
            </ul>
            ...
            <div class="tab-content">
              <div class="tab-pane fade show active" id="tab1" role="tabpanel">
                <table class="table stripe">
                  ...
                  <tbody>
                    <template v-for="item in transferRecords">
                      <tr :key="item.id" v-if="seek(item)">
                        ...
                        <td>{{ item.amount }}</td>
                        <td>
                          {{ $t(`wallet.${item.draw_out_direction}`) }} -> {{ $t(`wallet.${item.into_direction}`) }}
                        </td>
                        <td>{{ item.datetime | parseTime }}</td>
                        <td class="text-right">
                          <!--1 成功 2 失败 -->
                          <span class="fail" v-if="item.status == Globals.Status.Transfer.FAIL">{{
                            $t('wallet.error')
                          }}</span>
                          <span class="success" v-else-if="item.status == Globals.Status.Transfer.SUCCESS">{{
                            $t('wallet.success')
                          }}</span>
                        </td>
                      </tr>
