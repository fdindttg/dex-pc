  ...
  <div class="wallet-page">
    ...
    <div class="container">
      <div class="w-100 d-flex justify-content-between align-items-center mb-3 mt-4">
        ...
        <div class>
          <button type="button" class="btn btn-success" data-toggle="modal" data-target="#editAddress" @click="add">
          ...
          <div class="markets-pair-list mb-3">
            ...
            <div class="tab-content panel-box">
              <div class="tab-pane fade show active" id="tab1" role="tabpanel">
                <ul>
                  <li class="heading">
                    <span class="pr-2 border-right d-inline-block">{{ $t('wallet.assets') }}</span>
                    <span class="pr-2 d-inline-block">{{ $t('wallet.addressNum') }}</span>
                    <!-- <span class="pr-2 d-inline-block">{{ $t('wallet.fee1') }}</span> -->
                  </li>

                  <el-collapse accordion>
                    <el-collapse-item v-for="coin in userAdressList" :key="coin.coin_id">
                      ...
                      <table class="table stripe">
                        ...
                        <tbody>
                          <tr v-for="(address, index) in coin.list" :key="address.id">
                            <td>{{ index + 1 }}</td>
                            <td>
                              {{ address.datetime | parseTime }}
                            </td>
                            <td>
                              <span :id="`withdrawal-address-${address.id}`">
                                {{ address.address }}
                              </span>
                              <!-- 使用clipboard-target指定复制的元素 -->
                              <span
                                :class="`my-pop-${address.id}`"
                                data-toggle="popover"
                              ...
                              >
                                <a
                                  ...
                                  :data-clipboard-target="`#withdrawal-address-${address.id}`"
                                  alt="Copy to clipboard"
                                  :data-index="address.id"
                            ...
                            <td class="text-right">
                              ...
                              <button type="button" class="btn btn-sm btn-danger" @click="del(address.id)">
        ...
        <div class="modal-content bg-panel">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">
              {{ action == 'add' ? $t('wallet.addAddress') : $t('wallet.editAddress') }}
            </h5>
          ...
          <div class="modal-body">
            <form>
              <div class="form-group">
                <label for="recipient-assets" class="col-form-label">{{ $t('wallet.assets') }}:</label>
                <div>
                  <!-- 编辑时禁止选择币种 -->
                  <el-select
                  ...
                  >
                    <el-option label="USDT(TRC20)" value="1-3">
                      <img src="../../assets/img/icon/USDT.png" width="20" />
                      USDT(TRC20)
                    </el-option>
                    <el-option label="USDT(ERC20)" value="1-2">
                      <img src="../../assets/img/icon/USDT.png" width="20" />
                      USDT(ERC20)
                    </el-option>
                    <el-option label="USDT(OMNI)" value="1-1">
                      <img src="../../assets/img/icon/USDT.png" width="20" />
                      USDT(OMNI)
                    </el-option>
                    <template v-for="coin in coinsForWithdrawal">
                      <el-option
                      ...
                      >
                        <img :src="coin.image" width="20" />
                        {{ coin.coin_name }}(ERC20)
                      </el-option>
                      <el-option
                        :label="coin.coin_name"
                        :value="coin.coin_name"
                        :key="coin.coin_id"
                        v-if="coin.coin_id == 1070"
                      >
                        <img :src="coin.image" width="20" />
                        {{ coin.coin_name }}(ERC20)
                      </el-option>
              ...
              <div class="form-group">
                <label for="recipient-adress" class="col-form-label">{{ $t('wallet.drawAddress') }}:</label>
                <input type="text" class="form-control" id="recipient-adress" v-model="userAdress.address" />
              </div>
              <div class="form-group">
                <label for="comments" class="col-form-label">{{ $t('wallet.addressRemarks') }}:</label>
                <input type="text" class="form-control" id="comments" v-model="userAdress.address_note" />
              </div>
            </form>
          </div>
...
export default {
  ...
  methods: {
    ...
    handleUserAddress() {
      let pms
      let data = this.userAdress
      if (
        this.userAdress.coin_name == '1-1' ||
        this.userAdress.coin_name == '1-2' ||
        this.userAdress.coin_name == '1-3'
      ) {
        data.coin_name = 'USDT'
      }
      switch (this.action) {
        case 'add':
          pms = Wallet.addWithdrawAddress(data)
          break
        case 'edit':
