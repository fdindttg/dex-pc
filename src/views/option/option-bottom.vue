<template>
  <div class="markets-pair-list">
    <ul class="nav nav-pills" id="pills-tab" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" data-toggle="pill" href="#tabBottom1" role="tab" aria-selected="false">{{
          $t('option.waitFor')
        }}</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" data-toggle="pill" href="#tabBottom2" role="tab" aria-selected="false">{{
          $t('option.history')
        }}</a>
      </li>
    </ul>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="item in list2" :key="item.order_no">
              ...
              <td>{{ item.status_text }}</td>
              <td>
                <template v-if="item.end_price > 0">
                  {{ item.delivery_range }}% (
                  <span class="increace" v-if="item.end_price > item.scene.begin_price">{{ $t('option.up') }}</span>
                  <span class="decreace" v-if="item.end_price < item.scene.begin_price">{{ $t('option.down') }}</span>
                  <span v-if="item.end_price == item.scene.begin_price">{{ $t('option.flat') }}</span
                  >)
                </template>
                <template v-else>
                  {{ item.scene.delivery_range }}% (
                  <span class="increace" v-if="item.scene.delivery_up_down == 1">{{ $t('option.up') }}</span>
                  <span class="decreace" v-if="item.scene.delivery_up_down == 2">{{ $t('option.down') }}</span>
                  <span v-if="item.scene.delivery_up_down == 3">{{ $t('option.flat') }}</span
                  >)
                </template>
              </td>
              <td :class="item.delivery_amount * 1 > 0 ? 'increace' : 'decreace'">
...
export default {
  ...
  methods: {
    ...
    getOptionHistoryOrders(status, page, call) {
      ...
      if (status == 1) {
        data['time_id'] = this.activeCoin.time_id
      }
