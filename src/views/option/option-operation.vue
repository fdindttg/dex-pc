  ...
  <div class="opt-box">
    <div class="markets-pair-list">
      <ul class="nav nav-pills" id="pills-tab" role="tablist">
        <li class="nav-item">
          <a class="nav-link active" data-toggle="pill" href="#tab1" role="tab" aria-selected="false">{{
            $t('option.buyOption')
          }}</a>
        </li>
        <li class="nav-item">
      ...
      <div class="tab-content">
        <div class="tab-pane fade show active" id="tab1" role="tabpanel">
            ...
            <div class="item d-flex justify-content-between py-2">
              ...
              <div class="col">
                <time-count
                  isAuto
                  @end="resTimer"
                  :startTime="current_scene.lottery_time"
            ...
            <div class="item d-flex justify-content-between py-2">
              ...
              <div class="col">
                <time-count
                  isAuto
                  @end="resTimer"
                  :startTime="next_scene.lottery_time"
                  :benginTime="next_scene.begin_time"
                />
              </div>
            </div>
          </div>
          <div class="form-box p-2">
            ...
            <div class="form">
              <div class="item">
                <div class="head d-flex justify-content-between align-items-center px-3 py-1 bg-light rounded-pill">
                  <div>
                  ...
                  <div>
                    {{ $t('option.yield') }}
                    <span>{{ (activeUp.odds * 100).toFixed(0) }}%</span>
                  </div>
                </div>
                <div class="mt-3" id="lookUp">
                  <div class="input-group input-group-sm mb-3">
                    ...
                    <input
                      ...
                      step="0.000001"
                      v-model="upAmount"
                      :placeholder="$t('option.至少买入') + omitTo(current_scene.min_money, 0)"
                      type="number"
                      class="form-control"
                    />
                    <div class="input-group-append">
                      <button type="button" disabled class="btn btn-outline-secondary">{{ upCoin.coin_name }}</button>
                    </div>
                    <small class="w-100 form-text text-muted">
                      <span>{{ $t('option.balance') }}：{{ upCoin.balance || '--' }} {{ upCoin.coin_name }}</span>
                      <span class="increace float-right"
                        >{{ $t('option.expectedRevenue') }}：{{ expected(upAmount, activeUp.odds) }}</span
                      >
                    </small>
                  </div>
                </div>
              ...
              <div class="item mt-4">
                <div class="head d-flex justify-content-between align-items-center px-3 py-1 bg-light rounded-pill">
                  ...
                  <div>
                    {{ $t('option.yield') }}
                    <span>{{ (activeDown.odds * 100).toFixed(0) }}%</span>
                  </div>
                </div>
                <div class="mt-3" id="lookDown">
                  <div class="input-group input-group-sm mb-3">
                    ...
                    />
                    <div class="input-group-append">
                      <button type="button" disabled class="btn btn-outline-secondary">{{ downCoin.coin_name }}</button>
                    </div>
                    <small class="w-100 form-text text-muted">
              ...
              <div class="item mt-4">
                <div class="head d-flex justify-content-between align-items-center px-3 py-1 bg-light rounded-pill">
                  ...
                  <div>
                    {{ $t('option.yield') }}
                    <span>{{ (activeDraw.odds * 100).toFixed(0) }}%</span>
                  </div>
                </div>
                <div class="mt-3" id="lookDraw">
                  <div class="input-group input-group-sm mb-3">
                    ...
                    />
                    <div class="input-group-append">
                      <button type="button" disabled class="btn btn-outline-secondary">{{ drawCoin.coin_name }}</button>
                    </div>
                    <small class="w-100 form-text text-muted">
        ...
        <div class="tab-pane fade show bill-list" id="tab2" role="tabpanel">
          ...
          <ul class="list p-2" v-infinite-scroll="resGetSceneResultList">
            <li class="d-flex justify-content-between fn-12" v-for="item in list" :key="item.scene_id">
        ...
        .then((res) => {
          ...
          if (res.next_scene) {
            ...
            this.next_scene = {
              ...
              status_text: '-',
              lottery_time: 0,
              begin_time: this.next_scene.begin_time
            }
          }
        })
        .catch((err) => {})
