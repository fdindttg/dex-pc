          ...
          <template v-if="step === 1">
            <div class="form-group">
              <input
                ...
                required
                :data-has="$t('login.effectivePhoneEmail')"
                class="form-control"
                :placeholder="$t('login.pleaseEmail')"
                :data-message="$t('login.effectivePhoneEmail')"
              />
            </div>
            <div class="form-group">
              ...
              <el-input
                ...
                show-password
                required
                :data-has="$t('login.emailCode', { num: '8-15' })"
                pattern="(?=.*([a-zA-Z].*))(?=.*[0-9].*)[a-zA-Z0-9-*/+.~!@#$%^&*()]{8,15}$"
                :data-message="$t('login.passwordLength', { num: '8-15' })"
          ...
          <template v-else-if="step === 3">
            <div class="login-type" v-if="status.google_status + status.phone_status + status.email_status >= 2">
              ...
              <ul class="login-type__container">
                <li
                  v-if="status.google_status"
                  @click="checkType = 3"
                  :class="[`login-type__controls`, { active: checkType == 3 }]"
                >
                  Google
                </li>
                <li
                  v-if="status.phone_status"
                  @click="checkType = 1"
                  :class="[`login-type__controls`, { active: checkType == 1 }]"
                >
                  {{ $t('login.sms') }}
                </li>
                <li
                  v-if="status.email_status"
                  @click="checkType = 2"
                  :class="[`login-type__controls`, { active: checkType == 2 }]"
                >
                  {{ $t('login.email') }}
                </li>
              </ul>
            ...
            <div class="verify-box" v-for="(item, index) in codeLists" :key="index" v-show="item.type === checkType">
              ...
              <div v-if="item.resend" class="resend-btn">
                ...
                <verify-code
                  ...
                  :autoSend="item.type === checkType && isFirst[item.name]"
                ></verify-code>
              </div>
              ...
              <ul class="code-inputs">
                <li v-for="pos in 6" class="code-items" :key="pos">
                  <input
                    ...
                    placeholder
                    :ref="`space-${item.name}`"
                    @keyup="handleKeyUp(item.name, pos - 1, $event)"
                    @input="handleInput(item.name, pos - 1, $event)"
                    @paste="autofill"
                  />
                </li>
              </ul>
          ...
          <iframe
            ...
            :src="
              `/static/SliderCaptcha/src/index.html?` +
                encodeURIComponent(`nextStep=step2Login&slider_type=login&account=${user.account}&lang=${lang}`)
            "
            frameborder="0"
          ></iframe>
        </div>
  ...
  methods: {
    ...
    autofill($event) {
      ...
      if (val.match(/^\d{6}$/)) {
        ...
        let { name, inputList } = this.type2name()
    ...
    reset(checkType, autoFocus = false) {
      let { inputList, name } = this.type2name(checkType)
    ...
    step2(token) {
      ...
      Member.login(data)
        .then((response) => {
          let { google_status, phone_status, email_status, signature, phone, email } = response
          ...
          if (type === 1) {
            // 手机登录
            this.ajar.phone = phone.replace(/^(\d{3})\d{4}(\d{4})$/, '$1****$2')
          } else if (type === 2) {
            // 邮箱登陆
