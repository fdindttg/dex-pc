      ...
      <table class="table">
        <thead>
          <tr style="display:block;">
            <th class="w-33">{{ $t('exchange.price') }}({{ pair.from }})</th>
            <th class="w-33 text-right">{{ $t('exchange.amount') }}({{ pair.to }})</th>
        ...
        <tbody class="sell-orders">
          <div class="order-item" v-for="item in sellList" :key="item.id">
            <el-progress
              ...
              type="line"
              :stroke-width="39"
              :percentage="getValue(item.amount)"
              :show-text="false"
              color="rgba(216,43,43,0.15)"
            >
            </el-progress>
            <!-- 卖单 使用弹性盒子倒序排列 -->
        ...
        <tbody class="ob-heading">
          <tr style="display:block;" v-if="newTrade">
            ...
            <!-- <td class="w-33 text-right">
              <span>CNY</span>
              ≈ {{ omitTo(newTrade.price * price_cny) }} ≈ {{ (newTrade.price * priceCny) | omitTo(2) }}
            </td> -->
          ...
          <tr style="display:block;" v-else>
            ...
            <!-- <td class="w-33 text-right"><span>CNY</span> -</td> -->
            <td class="w-33 text-right">
              <span>{{ $t('exchange.change') }}</span> -
            </td>
          </tr>
            ...
            <thead>
              <tr>
                <th style="display:block;width:100%">
                  <div class="w-33">{{ $t('exchange.price') }}({{ pair.from }})</div>
                  <div class="w-33 text-right">{{ $t('exchange.amount') }}({{ pair.to }})</div>
...
export default {
  ...
  computed: {
    ...
    changeRate() {
      ...
      let val = Math.division(Math.subtr(this.newTrade.price, this.marketInfo.open), this.marketInfo.open)
