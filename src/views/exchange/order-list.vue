            ...
            <tbody class="order-tbody">
              <tr v-for="(item, idx) in ordersOpen.data" :key="item.id" class="text-nowrap">
                ...
                <td class="text-nowrap">
                  <button type="button" class="btn btn-sm btn-outline-danger" @click="delOrder(item, idx)">
                    {{ $t('common.cancel') }}
                  </button>
                </td>
              </tr>
            ...
            <tbody>
              <template v-for="(item, index) in ordersHistory.data">
                <tr
                  class="text-nowrap"
                  :key="item.id"
                  :name="item.entrust_type + '_' + item.id + '_' + index"
                  slot="title"
                  ...
                  <td colspan="11">
                    <template v-if="item.children && item.children.length">
                      <div v-for="rec in item.children" :key="rec.order_id">
                        <span>
                          <span class="text-secondary"> {{ $t('common.id') }} </span>：{{ rec.order_id }} &emsp;
                        </span>
                        <span
                          ><span class="text-secondary"> {{ $t('common.created') }}</span
                          >：{{ rec.created_at }} &emsp;</span
                        >
                        <span
                          ><span class="text-secondary"> {{ $t('common.filled-price') }}</span
                          >：{{ rec.unit_price | omitTo(priceDecimals) }} &emsp;</span
                        >
                        <span
                          ><span class="text-secondary"> {{ $t('common.filled-amount') }}</span
                          >：{{ rec.trade_amount | omitTo(qtyDecimals) }} &emsp;</span
                        >
                        <span
                          ><span class="text-secondary"> {{ $t('common.filled-total') }}</span
                          >：{{ rec.trade_money | omitTo(priceDecimals) }} &emsp;</span
                        >
                        <!-- <span
                          ><span class="text-secondary"> {{ $t('common.fee') }}</span
                          >：{{ rec.trade_fee | omitTo(priceDecimals) }} &emsp;</span
                        > -->
                      </div>
                    </template>
...
export default {
  props: ['ordersOpen', 'ordersHistory', 'conditionOrders', 'priceDecimals', 'qtyDecimals', 'isLogin', 'pair'],
  ...
  methods: {
    ...
    delOrder(item, idx) {
      ...
      //是否撤销当前订单
      this.$confirm('Please confirm to cancel?')
        .then((res) => {
