...
export default {
  ...
  watch: {
    ...
    marketInfo(newVal, oldVal) {
      if (newVal && !oldVal) {
        // 第一次取得marketInfo时 更新接口 主要针对刷新页面
        this.update()
      }
    },
  ...
  methods: {
    ...
    getBalance() {
      if (this.isLogin) {
        Exchange.getUserBalance(this.marketInfo.pair_name)
          .then((data) => {
  ...
  created() {
    ...
    this.ws.on('message', (response) => {
      ...
      switch (sub) {
        ...
        case `tradeList_${this.symbol}`:
          ...
          if (
            (this.newTrade && this.newTrade.buy_user_id === this.userInfo.user_id) ||
            (this.newTrade && this.newTrade.sell_user_id == this.userInfo.user_id)
          )
            this.update()
          break
      }
    ...
    this.ws.on('close', () => {
      this.$message({
        type: 'error',
        message: this.$t('common.errorPrompt'),
        duration: 2000
      })
    })
