...
export default {
  ...
  computed: {
    lang() {
      let arr = ['vi', 'en', 'de', 'tr']
      let local
      console.log(111, localStorage.lang)
      if (localStorage.lang) {
        if (localStorage.lang == 'tw') {
          local = 'zh_TW'
        } else if (localStorage.lang == 'cn') {
          local = 'zh'
        } else if (localStorage.lang == 'id') {
          local = 'id_ID'
          ...
          local = 'en'
        } else if (arr.includes(localStorage.lang) > -1) {
    ...
    createBtns() {
      ...
      resolutions.forEach((item) => {
        ...
        let $parent = widget
          ...
          .text(item.title)
          .on('click', function(e) {
            const btns = this.parentNode.parentNode.querySelectorAll('.my-date')
      ...
      widget.chart().createStudy('Moving Average', false, true, [30, 'close', 0], null, {
        'Plot.color': '#ff231f'
      })
